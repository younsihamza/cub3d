...
/*   Updated: 2023/06/21 21:45:32 by hyounsi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((vars->up == 1 || vars->f_up == 1) && check_pixel(vars,
			sin(vars->deriction) * vars->speed, cos(vars->deriction)
			* vars->speed) == 0)
	{
...
{
	help_move(vars);
	if ((vars->down == 1 || vars->f_down == 1) && check_pixel(vars,
			sin(vars->deriction) * vars->speed, cos(vars->deriction)
			* vars->speed) == 0)
	{
		vars->player_x -= cos(vars->deriction) * vars->speed;
